...
static uint32_t yModemSend(const char *filename, uint8_t *arraydata, uint32_t arraysize, int waitForReceiver, int batchMode )
{
  ...
  if (waitForReceiver)
  {
    Serial2.println("Waiting for receiver ping ...");
  ...
  if (answer != 'C')
  {
    Serial2.println("Timeout starting YModem transfer");
    return 0;
  }
  else
  {
  ...
  Serial2.print("Ymodem Sending ");
  Serial2.println(filename);
  Serial2.print(spfBuff);
  ...
  while (numBytesStillToSend > 0)
  {
    ...
    Serial.write( (uint8_t*) &yPacket, sizeof(yPacket));

    // wait for the receiver to send back a response to the packet
    startmS = millis();
    while ((!Serial.available()) && ((uint32_t) (millis() - startmS) < transfertimeoutmS));
    if (!Serial.available())
    {
      Serial2.println("Timeout waiting YModem response");
      return 0;
    }
    ...
    {
      Serial2.println("Ack error limit reached");
      return 0;
    }
