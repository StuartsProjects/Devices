...
/*******************************************************************************************************
  ...
  sending out a shorter Search mode binary location only payload. This is intended for ground based searching
  and locating. The shorter packet makes it practical to use long range settings. The frequency and LoRa settings
  of the Search mode packet can be different to the Tracker mode used by the HAB payload. To switch between
  standard tracker and long range search mode press the switch that is defined in the Settings.h file.

  The program will drive a SSD1306 or SH1106 OLED display for portable use.

  Note that the distance and direction to the tracker is only displayed when there has been at least one location
  fix from the remote tracker and a locally attached GPS has a fix.
...
//NeoSWSerial GPSserial(RXpin, TXpin);                      //NeoSWSerial library is more relaible with a GPS over SoftwareSerial
...
void loop()
{
  RXPacketL = LoRa.receiveSXBuffer(0, 0, NO_WAIT);
...
void packet_is_OK()
{
  ...
  if (PacketType == HABPacket)
  {
    ...
    actualCRC = LoRa.CRCCCITTSX(1, RXPacketL - 6, 0xFFFF);
...
uint16_t calcIncludedCRC()
{
  ...
  uint16_t crc;
  high = LoRa.getByteSXBuffer(RXPacketL - 4);
  midhigh = LoRa.getByteSXBuffer(RXPacketL - 3);
  midlow = LoRa.getByteSXBuffer(RXPacketL - 2);
  low = LoRa.getByteSXBuffer(RXPacketL - 1);
...
void printpacketDetails()
{
  ...
  hertzerror = LoRa.getFrequencyErrorHz();
...
void extractHABdata(uint8_t startaddr)
{
  ...
  do
  {
    buffData =  LoRa.getByteSXBuffer(ptr++);
  }
  while ( buffData == '$');
...
uint8_t extractBuffer(char *mybuffer, size_t bufferSize, uint8_t startptr)
{
  ...
  {
    mybuffer[index] = LoRa.getByteSXBuffer(startptr++);
...
uint8_t nextComma(uint8_t localpointer)
{
  ...
  do
  {
    bufferdata =  LoRa.getByteSXBuffer(localpointer++);
  }
...
int32_t extractUint(uint16_t localpointer)
{
  ...
  do
  {
    buffdata =  LoRa.getByteSXBuffer(localpointer++);;
...
void setup()
{
  ...
  Serial.println();
  Serial.println();
  Serial.println(F("9_Balloon_Tracker_Receiver Starting"));
  ...
  if (LoRa.begin(NSS, NRESET, DIO0, LORA_DEVICE))              //Initialize LoRa device
  {
